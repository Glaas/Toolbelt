...
public static class Easings
{
    ...
    public static float easeInSine(float x) { return 1 - Cos((x * PI) / 2); }
    public static float easeOutSine(float x) { return Sin((x * PI) / 2); }
    public static float easeInOutSine(float x) { return -(float)(Cos(PI * x) - 1) / 2; }
    ...
    public static float easeInExpo(float x) { return (x == 0) ? 0 : Pow(2, 10 * (x - 1)); }
    public static float easeOutExpo(float x) { return (x == 1) ? 1 : (-Pow(2, -10 * x) + 1); }
    ...
    public static float easeInOutExpo(float x)
    {
        ...
        if (x == 1) return 1;
        x *= 2;
        if (x < 1) return 0.5f * Pow(2, 10 * (x - 1));
        x--;
        return 0.5f * (-Pow(2, -10 * x) + 2);
    }

    public static float easeInCirc(float x) { return -(Sqrt(1 - (x * x)) - 1); }
    ...
    {
        x *= 2;
        if (x < 1) return -0.5f * (Sqrt(1 - x * x) - 1);
        x -= 2;
        return 0.5f * (Sqrt(1 - x * x) + 1);
    }
    ...
    public static float easeInElastic(float x)
    {
        ...
        float s = p / 4;
        x -= 1;
        return -(Pow(2, 10 * x) * Sin((x - s) * (2 * PI) / p));
    }
    ...
    public static float easeOutElastic(float x)
    {
        ...
        float p = 0.3f;
        float s = p / 4;
        return Pow(2, -10 * x) * Sin((x - s) * (2 * PI) / p) + 1;
    }
    ...
    public static float easeInOutElastic(float x)
    {
        ...
        float s = p / 4;
        x *= 2;
        if (x < 1) return -0.5f * (Pow(2, 10 * (x - 1)) * Sin((x - s) * (2 * PI) / p));
        x -= 1;
        return Pow(2, -10 * x) * Sin((x - s) * (2 * PI) / p) * 0.5f + 1;
    }
